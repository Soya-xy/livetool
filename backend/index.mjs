...
const updateManifestFile = resolve(process.env.UPDATE_MANIFEST_FILE || join(dirname(dataFile), 'updates', 'manifest.json'))
const updateArtifactDir = resolve(process.env.UPDATE_ARTIFACT_DIR || join(dirname(dataFile), 'updates', 'artifacts'))
...
async function route(request, response) {
  ...
  if (request.method === 'GET' && path === '/v1/updates/check') return checkUpdate(request, response)
  const updateArtifact = path.match(/^\/v1\/updates\/artifacts\/([A-Za-z0-9._-]+)$/)
  if (request.method === 'GET' && updateArtifact) return downloadUpdate(request, response, updateArtifact[1])
...
async function checkUpdate(request, response) {
  const session = requireUser(request)
  cardForSession(session)
  let manifest
  try {
    manifest = JSON.parse(await readFile(updateManifestFile, 'utf8'))
  } catch (error) {
    if (error?.code === 'ENOENT') { response.writeHead(204); return response.end() }
    throw new HttpError(503, '更新清单暂时不可用')
  }
  if (!manifest || typeof manifest.version !== 'string' || !Array.isArray(manifest.artifacts)) throw new HttpError(503, '更新清单格式无效')
  const platform = requestURL(request).searchParams.get('platform') || ''
  const arch = requestURL(request).searchParams.get('arch') || ''
  const hasArtifact = manifest.artifacts.some((item) => item && item.platform === platform && item.arch === arch)
  if (!hasArtifact) { response.writeHead(204); return response.end() }
  return json(response, 200, manifest)
}

async function downloadUpdate(request, response, filename) {
  const session = requireUser(request)
  cardForSession(session)
  if (basename(filename) !== filename) throw new HttpError(400, '更新文件名无效')
  const file = resolve(updateArtifactDir, filename)
  const artifactPath = relative(updateArtifactDir, file)
  if (isAbsolute(artifactPath) || artifactPath === '..' || artifactPath.startsWith(`..${sep}`)) throw new HttpError(400, '更新文件名无效')
  let info
  try { info = await stat(file) } catch (error) {
    if (error?.code === 'ENOENT') throw new HttpError(404, '更新文件不存在')
    throw error
  }
  if (!info.isFile()) throw new HttpError(404, '更新文件不存在')

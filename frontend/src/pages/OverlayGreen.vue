...
onMounted(() => {
  remove = api.overlay.onMessage((message) => {
    ...
    if (message.type === 'window-opacity') opacity.value = Number((message.payload as { opacity?: number }).opacity ?? 1)
  ...
  void api.overlay.status().then((list) => { opacity.value = list.find((item) => item.type === 'green')?.opacity ?? 1 }).catch(() => undefined)
  void api.overlay.ready('green')
...
<template>
  <div class="green-overlay" :style="{ opacity }">
    ...
    <div v-for="item in drops" :key="item.id" class="falling-item" :style="{ left: `${item.left}%`, top: `${item.top}px`, '--size': `${item.size}px`, '--rotate': `${item.rotate}deg` }">
      <img :src="item.image" alt="" aria-hidden="true" @error="($event.target as HTMLImageElement).style.display = 'none'"><span aria-hidden="true">✦</span>
    </div>
  </div>
</template>

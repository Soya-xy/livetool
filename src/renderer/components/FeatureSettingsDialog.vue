        ...
        <div v-if="draft.giftRules.length" class="gift-rule-list">
          <div v-for="rule in draft.giftRules" :key="rule.id" class="gift-rule-row">
            <div class="gift-rule-main">
              <el-input v-model="rule.giftName" placeholder="礼物名称" />
              <el-select v-model="rule.action" class="gift-action-select">
              ...
              <div v-for="field in inlineGiftFields()" :key="field.key" class="gift-rule-inline-value">
                ...
                />
                <el-button v-if="hasGiftRuleOverride(rule, field)" link type="info" @click="resetGiftRuleField(rule, field)">默认</el-button>
              </div>
              <el-switch v-model="rule.enabled" />
              <el-button link type="danger" @click="removeGiftRule(rule.id)">删除</el-button>
            ...
            <div v-if="isGiftRuleExpanded(rule.id) && fieldsForGiftRules().length" class="gift-rule-settings">
              <div v-for="field in fieldsForGiftRules()" :key="field.key" class="gift-rule-setting">
                ...
                <el-button v-if="hasGiftRuleOverride(rule, field)" link type="info" @click="resetGiftRuleField(rule, field)">恢复默认</el-button>
              </div>
            </div>
          </div>
        </div>
